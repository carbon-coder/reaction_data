...
file_path = Path('C:/Users/felix/Desktop/ML Programs/GIT/reaxys_data_project/Suzuki_Folder')
inputfile_name = 'Suzuki_SM_assigned_reduced.csv'
dataframe = pd.read_csv(file_path / inputfile_name)
    ...
    for j in range(0, len(smiles)):
        ...
        try:
            ...
            while len(Chem.MolFromSmiles(product[0]).GetSubstructMatches(M_Lig_double_SMARTS)) >= 1:
                    ...
                    product = [Chem.MolToSmiles(y) for y in rxn.RunReactants((Chem.MolFromSmiles(product[0]),))[0]]
                    print(i)
        ...
        if len(smiles_mol.GetSubstructMatches(M_Lig_double_SMARTS)) >= 1:
            dataframe.at[i, 'Additive'] = dataframe.at[i, 'Additive'].replace(smiles[j], '')
#p = dataframe.at[74, 'Additive']
#print(p)
print('Hello')
a2 = dataframe
print('Test2')
#print(dataframe.at[74, 'Catalyst'])
#print(dataframe.at[74, 'Additive'])
#print(dataframe.at[74, 'Ligand'])
...
#M-Lig -> Catalyst/Ligand/Additive !erstes Problem!
for i in progressbar.progressbar(range(0, len(dataframe.index))):
    smiles_import = dataframe.at[i, 'Additive'].split('.')
    smiles = []
    [smiles.append(x) for x in smiles_import if x not in smiles]
...
SMARTS_ligands = {
    ...
    'NHCs':['nc(=*)n', '[C,c]1[N,n][C,c][C,c][N,n]1c', 'C1N(CCN1[C,c])[C,c]', 'C1-,=N(CCN1[C,c])[C,c]'], #3 ist sehr allgemein (aliph. NHC core)
...
for i in progressbar.progressbar(range(0, len(dataframe.index))):
    ...
    for j in range(0, len(smiles)):
        ...
        if len(smiles_mol.GetSubstructMatches(Pd_corr_SMARTS)) >= 1:
            ...
            Pd_corr = [Chem.MolToSmiles(x) for x in rxn.RunReactants((Chem.MolFromSmiles(smiles[j]),))[0]]
...
print('Cleverhans!')
#Unify NHCs/Ligands
#[c,C,n,N,P;!#26,!#27,!#28,!#29,!#44,!#45,!#46,!#47,!#76,!#77,!#78,!#79]
dataframe.to_csv(file_path / 'Suzuki_final#1.csv', index=False)